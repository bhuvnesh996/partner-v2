...
export const fetchAssignmentsByUserId = async (userId: string) => {
  const response = await API.get(`/assignUniversity/user/${userId}`);
  return response.data;
};
...
export const universityGetOneQueryFn = async (id: string) => {
  const response = await API.get(`/university/getOne/${id}`);
  return response.data;
};

export const getCoursesQueryFn = async (params: any) => {
  const response = await API.get<any>("/course/search", { params });
  return response.data;
};



// Fetch universities with optional filters and pagination
export const needUniversitiesQueryFn = async (params: any) => {
  const response = await API.get<any>("/university/needAll", { params });
  return response.data;
};


export const getOneCoursesQueryFn = async (id: any) => {
  const response = await API.get<any>(`/course/getOne/${id}`);
  return response.data;
};


export const getAllPromotionalQueryFn = async (page: number, limit: number) => {
  const response = await API.get("/promotional/getAll", {
    params: { page, limit },
  });
  return response.data;
};
